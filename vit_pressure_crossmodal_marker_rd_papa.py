...
def _fit_ridge_predict(x_train: np.ndarray, y_train: np.ndarray, x_pred: np.ndarray, alpha: float) -> np.ndarray:
...
def fit_marker_proxy(
    ...
    pred_test = _fit_ridge_predict(x_ar_train[valid], y_m[valid], x_ar_test, alpha)
    pred_train_full = _fit_ridge_predict(x_ar_train[valid], y_m[valid], x_ar_train, alpha)
    ...
    if source_mode == "observed":
        ...
        for sid in pd.unique(sids):
            ...
            if int(tr_mask.sum()) >= min_rows and len(pd.unique(sids[tr_mask])) >= max(2, min_subjects - 1):
                pred_train[val_mask] = _fit_ridge_predict(x_ar_train[tr_mask], y_m[tr_mask], x_ar_train[val_mask], alpha)
            else:
...
class SafeProbClassifier:
    def __init__(self, kind: str = "logreg", C: float = 1.0, max_iter: int = 1500):
...
def train_preference_gate(blocks_train: OrderedDict[str, Tuple[np.ndarray, np.ndarray]], y_train: np.ndarray, subject_ids: np.ndarray, quality_train: np.ndarray, global_classes: np.ndarray, args) -> Tuple[Optional[BaseEstimator], Dict[str, Any]]:
    ...
    for pseudo in list(pd.unique(sids)):
        ...
        for ei, name in enumerate(expert_names):
            ...
            clf = SafeProbClassifier(kind=args.mard_expert_classifier, C=args.mard_logreg_c, max_iter=args.mard_logreg_max_iter)
...
def add_marker_rd_args(parser) -> None:
    ...
    parser.add_argument("--marker-expert-classifier", dest="mard_expert_classifier", default="logreg", choices=["logreg", "lda"])
    ...
    parser.add_argument("--mard-expert-classifier", dest="mard_expert_classifier", choices=["logreg", "lda"], help=argparse.SUPPRESS)

...
# -----------------------------------------------------------------------------
# run_cra_papa_profile_cuda_comparisons.sh
#
# Runs four CRA-PAPA profile comparisons against the CUDA-heavy profile script:
#
#   A. patched default
...
#
# Example:
#   bash run_cra_papa_profile_cuda_comparisons.sh
#
# Smoke test:
...
#
# Common overrides:
#   SCRIPT=vit_pressure_crossmodal_cra_papa_profile_patched.py
...
SCRIPT="${SCRIPT:-vit_pressure_crossmodal_cra_papa_profile_cuda.py}"
...
CRA_EXPERT_CLASSIFIER="${CRA_EXPERT_CLASSIFIER:-torch_logreg}"
CRA_RIDGE_BACKEND="${CRA_RIDGE_BACKEND:-torch}"
CRA_GATE_BACKEND="${CRA_GATE_BACKEND:-torch}"
CRA_TORCH_CLF_LR="${CRA_TORCH_CLF_LR:-1e-2}"
CRA_TORCH_CLF_BATCH_SIZE="${CRA_TORCH_CLF_BATCH_SIZE:-4096}"
CRA_TORCH_CLF_WEIGHT_DECAY="${CRA_TORCH_CLF_WEIGHT_DECAY:-0.0}"
CRA_TORCH_GATE_EPOCHS="${CRA_TORCH_GATE_EPOCHS:-200}"
CRA_TORCH_GATE_LR="${CRA_TORCH_GATE_LR:-1e-2}"
CRA_NUM_WORKERS="${CRA_NUM_WORKERS:-2}"
CRA_PREFETCH_FACTOR="${CRA_PREFETCH_FACTOR:-2}"
CRA_PIN_MEMORY="${CRA_PIN_MEMORY:-1}"
...
OUT_ROOT="${OUT_ROOT:-runs/cra_papa_profile_cuda_compare/${STAMP}}"
...
COMMON_ARGS=(
    ...
    --cra-ridge-backend "${CRA_RIDGE_BACKEND}"
    --cra-gate-backend "${CRA_GATE_BACKEND}"
    --cra-torch-clf-lr "${CRA_TORCH_CLF_LR}"
    --cra-torch-clf-batch-size "${CRA_TORCH_CLF_BATCH_SIZE}"
    --cra-torch-clf-weight-decay "${CRA_TORCH_CLF_WEIGHT_DECAY}"
    --cra-torch-gate-epochs "${CRA_TORCH_GATE_EPOCHS}"
    --cra-torch-gate-lr "${CRA_TORCH_GATE_LR}"
    --cra-num-workers "${CRA_NUM_WORKERS}"
    --cra-prefetch-factor "${CRA_PREFETCH_FACTOR}"
...
if [ "${CRA_PIN_MEMORY}" = "1" ]; then
    COMMON_ARGS+=(--cra-pin-memory)
fi
COMMON_ARGS+=(--cra-enable-tf32)
...
echo "[INFO] CUDA backends: expert=${CRA_EXPERT_CLASSIFIER} ridge=${CRA_RIDGE_BACKEND} gate=${CRA_GATE_BACKEND} embed_batch=${EMBED_BATCH_SIZE}"

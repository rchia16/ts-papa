...
def build_cra_loaders_with_subject_ids(subject: str, subjects: List[str], args) -> Tuple[DataLoader, DataLoader, np.ndarray, np.ndarray]:
    ...
    # CRA profile evaluation is feature-extraction heavy.  Keep these loaders
    # configurable so large downstream batches can be staged efficiently for CUDA.
    # This does not affect the original pretraining loaders created by the core
    # runner, but it removes an avoidable CPU bottleneck in the CRA post hook.
    num_workers = int(getattr(args, "cra_num_workers", getattr(args, "num_workers", 0)))
    pin_memory = bool(getattr(args, "cra_pin_memory", False)) and str(getattr(args, "device", "")).startswith("cuda")
    loader_kwargs = dict(
    ...
    if num_workers > 0:
        ...
        loader_kwargs["prefetch_factor"] = int(getattr(args, "cra_prefetch_factor", 2))
...
def fit_ecg_proxy(x_ar_train: np.ndarray, ecg_train: np.ndarray, valid_train: np.ndarray, x_ar_test: np.ndarray, args) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict[str, Any]]:
    ...
    model = _make_ridge_model(args, alpha=float(args.cra_ecg_proxy_alpha))
...
def fit_ecg_profile_proxy(
    ...
    full_model = _make_ridge_model(args, alpha=float(getattr(args, "cra_profile_alpha", 10.0)))
    ...
    if source_mode == "observed":
        ...
        for i, ok in enumerate(valid_prof):
            ...
            if int(loo.sum()) >= min_subjects:
                mdl = _make_ridge_model(args, alpha=float(getattr(args, "cra_profile_alpha", 10.0)))
...
# -----------------------------------------------------------------------------
# CUDA-backed classical heads
# -----------------------------------------------------------------------------
class TorchRidgeRegressor:
    """Small GPU ridge regressor with StandardScaler-like input normalization.

    Used for AR -> ECG profile/proxy regression.  The output remains NumPy so the
    rest of the CRA-PAPA code can stay unchanged, but the expensive linear solve
    and predictions can run on CUDA.
    """

    def __init__(self, alpha: float = 10.0, device: str = "cuda:0", dtype: torch.dtype = torch.float32):
        ...
        self.x_scale_: Optional[torch.Tensor] = None
    ...
    def fit(self, x: np.ndarray, y: np.ndarray):
        ...
        y_np = np.nan_to_num(np.asarray(y, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
        if y_np.ndim == 1:
            y_np = y_np.reshape(-1, 1)
        x_t = torch.as_tensor(x_np, dtype=self.dtype, device=self.device)
        y_t = torch.as_tensor(y_np, dtype=self.dtype, device=self.device)
        self.x_mean_ = x_t.mean(dim=0, keepdim=True)
        self.x_scale_ = x_t.std(dim=0, unbiased=False, keepdim=True).clamp_min(1e-6)
        z = (x_t - self.x_mean_) / self.x_scale_
        ones = torch.ones((z.size(0), 1), dtype=z.dtype, device=z.device)
        za = torch.cat([z, ones], dim=1)
        eye = torch.eye(za.size(1), dtype=z.dtype, device=z.device)
        eye[-1, -1] = 0.0  # do not regularize intercept
        lhs = za.T @ za + self.alpha * eye
        rhs = za.T @ y_t
        try:
            self.coef_ = torch.linalg.solve(lhs, rhs)
        except RuntimeError:
            self.coef_ = torch.linalg.pinv(lhs) @ rhs
        return self

    @torch.no_grad()
    def predict(self, x: np.ndarray) -> np.ndarray:
        if self.x_mean_ is None or self.x_scale_ is None or self.coef_ is None:
            raise RuntimeError("TorchRidgeRegressor must be fitted before predict().")
        x_np = np.nan_to_num(np.asarray(x, dtype=np.float32), nan=0.0, posinf=0.0, neginf=0.0)
        x_t = torch.as_tensor(x_np, dtype=self.dtype, device=self.device)
        z = (x_t - self.x_mean_) / self.x_scale_
        ones = torch.ones((z.size(0), 1), dtype=z.dtype, device=z.device)
        pred = torch.cat([z, ones], dim=1) @ self.coef_
        return pred.detach().cpu().numpy().astype(np.float32)


class TorchProbClassifier:
    """CUDA multinomial logistic regression with predict_proba API."""
    ...
    def __init__(
        ...
        self.x_mean_: Optional[torch.Tensor] = None
        self.x_scale_: Optional[torch.Tensor] = None
        self.weight_: Optional[torch.Tensor] = None
        self.bias_: Optional[torch.Tensor] = None
    ...
    def fit(self, x: np.ndarray, y: np.ndarray):
        ...
        if len(self.classes_) <= 1:
            ...
            return self
        cls_to_i = {int(c): i for i, c in enumerate(self.classes_)}
        yi_np = np.asarray([cls_to_i[int(v)] for v in y_np], dtype=np.int64)
        x_t = torch.as_tensor(x_np, dtype=torch.float32, device=self.device)
        y_t = torch.as_tensor(yi_np, dtype=torch.long, device=self.device)
        self.x_mean_ = x_t.mean(dim=0, keepdim=True)
        self.x_scale_ = x_t.std(dim=0, unbiased=False, keepdim=True).clamp_min(1e-6)
        z = (x_t - self.x_mean_) / self.x_scale_
        linear = torch.nn.Linear(z.size(1), len(self.classes_)).to(self.device)
        counts = np.bincount(yi_np, minlength=len(self.classes_)).astype(np.float32)
        weights = counts.sum() / np.maximum(counts, 1.0)
        weights = weights / max(float(weights.mean()), 1e-6)
        criterion = torch.nn.CrossEntropyLoss(weight=torch.as_tensor(weights, dtype=torch.float32, device=self.device))
        # Keep explicit weight decay small by default; C is mapped to an L2 term.
        wd = self.weight_decay + (1.0 / max(self.C, 1e-6)) * 1e-4
        opt = torch.optim.AdamW(linear.parameters(), lr=self.lr, weight_decay=wd)
        bs = max(16, min(self.batch_size, z.size(0)))
        for _ in range(max(1, self.max_iter)):
            perm = torch.randperm(z.size(0), device=self.device)
            for st in range(0, z.size(0), bs):
                idx = perm[st : st + bs]
                opt.zero_grad(set_to_none=True)
                loss = criterion(linear(z[idx]), y_t[idx])
                loss.backward()
                opt.step()
        self.weight_ = linear.weight.detach().clone()
        self.bias_ = linear.bias.detach().clone()
        self.constant_ = None
        return self
    ...
    def predict_proba(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.x_mean_ is None or self.x_scale_ is None or self.weight_ is None or self.bias_ is None:
            raise RuntimeError("TorchProbClassifier must be fitted before predict_proba().")
        outs = []
        bs = max(1024, self.batch_size)
        for st in range(0, x_np.shape[0], bs):
            xb = torch.as_tensor(x_np[st : st + bs], dtype=torch.float32, device=self.device)
            z = (xb - self.x_mean_) / self.x_scale_
            logits = F.linear(z, self.weight_, self.bias_)
...
class TorchBinaryGate(TorchProbClassifier):
    """Binary specialization used as the expert-reliability gate."""

    pass


def _make_ridge_model(args, alpha: float):
    backend = str(getattr(args, "cra_ridge_backend", "sklearn")).lower()
    if backend == "torch":
        return TorchRidgeRegressor(alpha=alpha, device=str(getattr(args, "device", "cuda:0")))
...
class SafeProbClassifier:
    ...
    def __init__(
        ...
        *,
        ...
        self.max_iter = int(max_iter)
        self.device = device
        self.torch_lr = float(torch_lr)
        self.torch_batch_size = int(torch_batch_size)
        self.torch_weight_decay = float(torch_weight_decay)
        self.classes_: np.ndarray = np.asarray([], dtype=int)
        self.constant_: Optional[int] = None
    ...
    def fit(self, x: np.ndarray, y: np.ndarray):
        ...
        try:
            if self.kind == "torch_logreg":
                self.model = TorchProbClassifier(
                    C=self.C,
                    max_iter=self.max_iter,
                    lr=self.torch_lr,
                    batch_size=self.torch_batch_size,
...
def train_preference_gate(blocks_train: OrderedDict[str, Tuple[np.ndarray, np.ndarray]], y_train: np.ndarray, subject_ids: np.ndarray, quality_train: np.ndarray, global_classes: np.ndarray, args) -> Tuple[Optional[BaseEstimator], Dict[str, Any]]:
    ...
    for pseudo in unique_sids:
        ...
        for ei, name in enumerate(expert_names):
            ...
            clf = SafeProbClassifier(kind=args.cra_expert_classifier, C=args.cra_logreg_c, max_iter=args.cra_logreg_max_iter, device=str(getattr(args, "device", "cuda:0")), torch_lr=float(getattr(args, "cra_torch_clf_lr", 1e-2)), torch_batch_size=int(getattr(args, "cra_torch_clf_batch_size", 4096)), torch_weight_decay=float(getattr(args, "cra_torch_clf_weight_decay", 0.0)))
    ...
    if str(getattr(args, "cra_gate_backend", "sklearn")).lower() == "torch":
        gate = TorchBinaryGate(
            C=float(args.cra_gate_c),
            max_iter=int(getattr(args, "cra_torch_gate_epochs", args.cra_gate_max_iter)),
            lr=float(getattr(args, "cra_torch_gate_lr", 1e-2)),
            batch_size=int(getattr(args, "cra_torch_clf_batch_size", 4096)),
            weight_decay=float(getattr(args, "cra_torch_clf_weight_decay", 0.0)),
...
def add_cra_args(parser) -> None:
    ...
    parser.add_argument("--cra-expert-classifier", default="logreg", choices=["logreg", "lda", "torch_logreg"])
    ...
    parser.add_argument("--cra-ridge-backend", default="sklearn", choices=["sklearn", "torch"], help="Backend for ECG proxy/profile Ridge regressions.")
    parser.add_argument("--cra-gate-backend", default="sklearn", choices=["sklearn", "torch"], help="Backend for the source-LOSO reliability gate.")
    parser.add_argument("--cra-torch-clf-lr", type=float, default=1e-2, help="Learning rate for --cra-expert-classifier torch_logreg and torch gate.")
    parser.add_argument("--cra-torch-clf-batch-size", type=int, default=4096, help="CUDA mini-batch size for torch_logreg experts/gate.")
    parser.add_argument("--cra-torch-clf-weight-decay", type=float, default=0.0)
    parser.add_argument("--cra-torch-gate-epochs", type=int, default=200, help="Epochs for --cra-gate-backend torch. Uses --cra-gate-max-iter when not set by user scripts.")
    parser.add_argument("--cra-torch-gate-lr", type=float, default=1e-2)
    parser.add_argument("--cra-num-workers", type=int, default=0, help="Num workers for CRA downstream feature-extraction loaders.")
    parser.add_argument("--cra-pin-memory", action="store_true", help="Pin CRA downstream DataLoader memory when using CUDA.")
    parser.add_argument("--cra-prefetch-factor", type=int, default=2)
    parser.add_argument("--cra-enable-tf32", dest="cra_enable_tf32", action="store_true", default=True, help="Enable TF32 matmul/cudnn for faster CUDA training on Ampere+ GPUs.")
    parser.add_argument("--cra-disable-tf32", dest="cra_enable_tf32", action="store_false")
...
def finalize_args_cra(args) -> None:
    ...
    if bool(getattr(args, "cra_enable_tf32", True)) and torch.cuda.is_available():
        try:

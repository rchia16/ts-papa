...
MARKER_SCRIPT="${MARKER_SCRIPT:-vit_pressure_crossmodal_marker_rd_papa.py}"
...
MARKER_EXPERT_CLASSIFIER="${MARKER_EXPERT_CLASSIFIER:-logreg}"
...
run_cmd() {
    ...
    mkdir -p "${out_dir}"
    local cmd=("${PYTHON_BIN}" "${script}" "${COMMON_ARGS[@]}" --device "${device}" --out-dir "${out_dir}" "$@")
